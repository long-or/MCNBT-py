...
def buffer_read(buffer, length, msg):
    ...
    if len(byte) != length: raise NbtParseError("数据可能被截断了 数据长度应该为 %s 实际为 %s 数据类型为 %s" % (len(byte), length, msg))
    return byte
