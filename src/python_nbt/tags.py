...
from . import TAGLIST, Codec as ce
from . import TAG
from .error import *
...
class BaseTag:
    ...
    @classmethod
    def from_bytes(cls, buffer): pass

    @classmethod
    def from_snbt(cls, buffer): pass
...
class TAG_Number(BaseTag):
    ...
    def __lt__(self, other):
        return self.get_value() < TAG_Number.try_to_number(other)
    ...
    def __add__(self, other):
        return self.__class__(self.get_value() + TAG_Number.try_to_number(other))
    ...
    def __divmod__(self, other):
        q = self.__class__(self.get_value() // TAG_Number.try_to_number(other))
        r = self.__class__(self.get_value() % TAG_Number.try_to_number(other))
        return (q, r)
    
    def __pow__(self, other, modulo=None):
        res = self.get_value() ** TAG_Number.try_to_number(other)
...
class TAG_List(BaseTag):
    ...
    def _from_snbtIO(cls, buffer):
        ...
        while True:
            ...
            if is_number:
                ...
                if value is None: raise SnbtParserError("无法解析的数字 '%s' 位于第%s行 第%s个字符到第%s个字符" % (token[1], *get_line(buffer.code, token[2])))
                res.append(value)
                continue
            value = buffer.parse_value(token)
            if value.type == Type:
    ...
    def to_snbt(self, format=False, size=4):
        ...
        if not 1 <= size <= 16: raise ValueError("")
        if format:
            buffer = StringIO()
    ...
    def set_type(self, type):
        if isinstance(type, TAG):
            ...
            raise TypeError("期望类型为 %s，但传入了 %s" % ((tuple([TAG] + list(TAGLIST.values))), type.__class__))
...
class TAG_Array(ABC, BaseTag):
    ...
    def __add__(self, other):
        if isinstance(other, TAG_Array):
            ...
            return self.__class__(self.__value + value)
        else:
